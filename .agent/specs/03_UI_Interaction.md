...
- **初期ローディング画面**:
  - アプリケーション起動時、リソース読み込み完了までローディング画面（スピナー）を表示する。
  - **解除条件**: 以下の条件をすべて満たした時点でフェードアウトする。
    1. Reactアプリケーションのマウント完了
    2. シナリオデータのロード完了（ノードが存在するか、新規プロジェクトとして初期化済み）
    3. Webフォントのロード完了（レイアウトシフト防止のため必須）
- **初回アクセス (Viewport)**: 保存されたビューポートデータが存在しない場合、キャンバス上の全ノードが画面に収まるように自動調整（`Fit View`）を行う。
